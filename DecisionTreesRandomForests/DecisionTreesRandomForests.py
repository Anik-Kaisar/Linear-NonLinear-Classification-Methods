...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_enc, random_state=50, test_size=0.3
    )
    ...
    pca_train_df = pd.DataFrame(
        data=principalComponents_train,
        columns=["principal component 1", "principal component 2"],
    )
    pca_test_df = pd.DataFrame(
        data=principalComponents_test,
        columns=["principal component 1", "principal component 2"],
    )
    ...
    print(
        "Test data accuracy after PCA:",
        accuracy_score(y_true=y_test, y_pred=y_pred_pca),
    )
    Utils.visualize_classifier(
        clf_pca, pca_test_df.values, y_test, ax=None, cmap="rainbow"
    )
    ...
    print(
        "Test data accuracy after Random Forest:",
